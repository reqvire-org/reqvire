...
1. **Check current model context:**
   ```bash
   npx -y "${REQVIRE_NPX_PACKAGE:-@reqvire-org/reqvire@latest}" --workspace "$PWD" search --filter-type="requirement" --json | jq -r '.global_counters.total_elements'
   npx -y "${REQVIRE_NPX_PACKAGE:-@reqvire-org/reqvire@latest}" --workspace "$PWD" coverage --json | jq -r '.summary.leaf_requirements_coverage_percentage'
   ```
...
3. **Draft the requirement content:**
...
4. **Add the requirement using reqvire add command:**
   ```bash
...
5. **Check if verification is needed:**
   ...
   - **Capability**: Coverage rolls up from verified requirements that specify it
...
7. **Next steps:**
   - If **leaf requirement**: Use [AddVerification](AddVerification.md) to create verification
   - If **parent requirement**: Explain verification will roll up from child requirements

## Element Manipulation

After adding requirements, you may need to reorganize. See [Move](Move.md) and [Remove](Remove.md).
...
- **Leaf verification**: Leaf requirements need direct verification; capability coverage rolls up from verified requirements
...
- **Capabilities** inherit verification coverage from requirements that specify them
...
- Keep exact commands, fields, URI patterns, workflow steps, outputs, file paths, and reject/write/emit behavior in compatible requirement-owned `source`, `specification`, `constraint`, `behavior`, `state`, and `input-output` contracts
- For complete capability (requirement + verification + test), use [AddCapability](AddCapability.md)
