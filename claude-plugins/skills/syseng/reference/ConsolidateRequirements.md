...
# Find requirements with contract_bindings (candidates for conversion to relations)
...
- Contract Bindings may need conversion to `definedBy` relations
...
### Step 3: Convert Contract Bindings to definedBy Where Appropriate
...
```bash
# Remove contract_bindings from requirement
...
- Verification mapping becomes blurrier: tests/evidence are bound to a combined requirement rather than the specific concern.
...
6. **Keep Metadata and Contract Bindings** - Preserve unchanged
