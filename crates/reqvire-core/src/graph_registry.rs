...
fn concept_validation_local_name(name: &str) -> String {
...
impl GraphRegistry {
    ...
    fn validate_contract_bindings(&self) -> Result<Vec<ReqvireError>, ReqvireError> {
    ...
    fn validate_concept_elements(&self) -> Result<Vec<ReqvireError>, ReqvireError> {
    ...
    fn concept_scheme_namespace_context(&self, scheme: &Element) -> Option<(String, String)> {
    ...
    fn materialize_concept_payload_context(&mut self) {
    ...
    fn concept_scheme_context_id(&self, element_id: &str) -> Option<String> {
        let mut visited = BTreeSet::new();
    ...
    fn concept_link_resolves_to_type<F>(&self, target: &str, predicate: F) -> bool
    where
        F: Fn(&Element) -> bool,
    {
        self.nodes
    ...
    fn element_to_markdown_with_context(
        ...
        let mut seen_contract_bindings: std::collections::HashSet<String> =
    ...
    pub fn merge_elements(
        ...
        let mut seen_contract_bindings: HashSet<String> = HashSet::new();
...
fn merge_ontology_blocks_into_target(
    ...
    let target_block = crate::parser::extract_single_fenced_subsection(target_content, "Ontology")
        .into_iter()
        .next()
        .ok_or_else(|| {
    ...
    for source_element in source_elements {
        let Some(source_block) =
            crate::parser::extract_single_fenced_subsection(&source_element.content, "Ontology")
                .into_iter()
                .next()
        else {
            continue;
