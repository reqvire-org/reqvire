...
pub enum ManualReviewIssue {
    ...
    #[serde(rename = "concept_authoring_warning")]
    ConceptAuthoringWarning {
        element: ElementInfo,
        warning: String,
        rationale: String,
    },
        ...
        if show_auditable && !self.needs_manual_review.is_empty() {
            ...
            for issue in &self.needs_manual_review {
                match issue {
                    ...
                    ManualReviewIssue::ConceptAuthoringWarning {
                        element,
                        warning,
                        rationale,
                    } => {
                        println!("### Concept Authoring Warning\n");
                        println!("**Element: {}**", element.name);
                        println!("File: [{}]({})\n", element.identifier, element.identifier);
...
pub fn analyze_model(registry: &GraphRegistry) -> LintReport {
    ...
    // Detect Markdown-native concept authoring warnings (needs manual review)
    needs_manual_review.extend(detect_concept_authoring_warnings(registry));
    ...
    needs_manual_review.sort_by(|a, b| {
        let key_a = match a {
            ...
            ManualReviewIssue::ConceptAuthoringWarning { element, .. } => {
                element.identifier.clone()
            }
        };
        let key_b = match b {
...
fn build_requirement_root_assignment(
    ...
    for requirement_id in &requirement_ids {
        let roots = resolve_root_candidates(
            requirement_id,
            &parent_map,
            &mut memo,
...
fn detect_concept_authoring_warnings(registry: &GraphRegistry) -> Vec<ManualReviewIssue> {
    let mut issues = Vec::new();
    let top_concept_ids = native_top_concept_ids(registry);
    ...
    for element in registry.get_all_elements() {
        if element.element_type.is_concept_scheme() {
            ...
            if scheme.definition.is_none() {
                issues.push(concept_authoring_warning(
                    element,
                    "missing-definition",
                    "Concept schemes should include main body text so humans, AI workflows, and semantic exports can understand the thesaurus boundary.",
                ));
            }
            continue;
        }
        ...
        if element.element_type.is_concept() {
            ...
            if concept.definition.is_none() {
                issues.push(concept_authoring_warning(
                    element,
                    "missing-definition",
                    "Concepts should include main body text so generated SKOS has explicit meaning.",
                ));
            }
            if concept.broader.is_empty()
                && concept.narrower.is_empty()
                && concept.related.is_empty()
                && !top_concept_ids.contains(&element.identifier)
            {
                issues.push(concept_authoring_warning(
                    element,
                    "isolated-concept",
                    "Concepts should normally have broader, narrower, related, or top-concept context so thesaurus navigation is connected.",
                ));
            }
        }
    }
...
fn native_top_concept_ids(registry: &GraphRegistry) -> FxHashSet<String> {
    ...
    {
        let Some(payload) = &scheme.concept_scheme else {
            continue;
        };
