...
impl ChangeImpactReport {
    ...
    pub fn to_text(&self, base_url: &str, git_commit: &str, previous_git_commit: &str) -> String {
        ...
        for elem in &self.changed {
            ...
            // Render contract_bindings with 📎 icon (and ⚠️ for changed ones)
            let contract_bindings = &elem.change_impact_tree.element.contract_bindings;
            for att in contract_bindings {
                let att_target = att.target.as_str();
                let change_icon = if elem.changed_contract_bindings.contains(&att_target) {
                    " ⚠️"
                } else {
                    ""
                };
            ...
            let rendered_tree = render_change_impact_tree(
                ...
                git_commit,
                new_element_ids,
                &elem.changed_contract_bindings,
            );
            if !rendered_tree.trim().is_empty() {
                output.push_str(&rendered_tree);
            }
...
/// Format contract_bindings URL for rendering
fn format_contract_bindings_url(
    target: &element::ContractBindingTarget,
    _element_file: &str,
    base_url: &str,
    git_commit: &str,
) -> String {
    match target {
        element::ContractBindingTarget::FilePath(path) => {
            format!(
                "{}/blob/{}/{}",
                base_url,
                git_commit,
        ...
        element::ContractBindingTarget::ElementIdentifier(id) => {
            format!("{}/blob/{}/{}", base_url, git_commit, id)
        }
    }
}

/// Format contract_bindings name for rendering (extract just the name part)
fn format_contract_binding_name(target: &element::ContractBindingTarget) -> String {
    match target {
        element::ContractBindingTarget::FilePath(path) => path.to_string_lossy().to_string(),
        element::ContractBindingTarget::ElementIdentifier(id) => {
...
fn render_change_impact_tree(
    ...
    for relation_node in &node.relations {
        ...
        // Render contract_bindings for child elements
        for att in &target.contract_bindings {
            let att_target = att.target.as_str();
            let att_change_icon = if changed_contract_bindings.contains(&att_target) {
                " ⚠️"
            } else {
                ""
            };
            ...
            output.push_str(&format!(
                "{}    * 📎 [{}]({}){}\n",
                pad, contract_binding_name, att_url, att_change_icon
            ));
        }
...
pub fn build_change_impact_tree(
    ...
    let mut relations: Vec<RelationNode> = impact_relations
        .into_iter()
...
fn maps_to_concept_impacted_ontology_elements(
    registry: &graph_registry::GraphRegistry,
    element: &element::Element,
) -> Vec<String> {
    const REQVIRE_MAPS_TO_CONCEPT: &str = "https://www.reqvire.org/ontology#mapsToConcept";

    let concept_iris = impacted_concept_iris(registry, element);
    if concept_iris.is_empty() {
        return Vec::new();
    }

    let semantic_index = semantic_contract::build_semantic_index(registry);
    let mut impacted = BTreeSet::new();
    for block in semantic_index
        .blocks
        .iter()
        .filter(|block| matches!(block.kind, SemanticBlockKind::Ontology))
    {
        if block.quads.iter().any(|quad| {
            quad.predicate.as_str() == REQVIRE_MAPS_TO_CONCEPT
                && term_iri(&quad.object)
                    .is_some_and(|iri| concept_iris.iter().any(|concept_iri| concept_iri == iri))
        }) && registry.get_element(&block.source).is_some()
        {
            impacted.insert(block.source.clone());
        }
    }
    impacted.into_iter().collect()
}

fn impacted_concept_iris(
    registry: &graph_registry::GraphRegistry,
    element: &element::Element,
) -> BTreeSet<String> {
    let mut iris = BTreeSet::new();
    if let Some(concept) = &element.concept {
        iris.insert(concept.iri.clone());
    }
    if let Some(scheme) = &element.concept_scheme {
        for candidate in registry.get_all_elements() {
            if let Some(concept) = &candidate.concept {
                if concept.scheme_iri.as_deref() == Some(scheme.iri.as_str()) {
                    iris.insert(concept.iri.clone());
                }
            }
        }
...
pub fn apply_smart_filtering(
    ...
    for added in &report.added {
        ...
        // Also collect contract_bindings element identifiers
        collect_contract_bindings_element_ids(
    ...
    // Step 3: Also collect contract_bindings from children in the tree
    for added in &report.added {
        collect_contract_binding_ids_from_tree(&added.change_impact_tree, &mut referenced_ids);
    }
    for changed in &report.changed {
        collect_contract_binding_ids_from_tree(&changed.change_impact_tree, &mut referenced_ids);
    }

    // Step 4: Filter out added/changed elements that are referenced elsewhere (not root).
    // This includes elements referenced via downstream relations OR via contract_bindings.
    report
        .added
...
/// Get contract_bindings hashes for an element, resolving element contract_bindings hashes from registry
/// Returns a sorted list of (contract_bindings_target_string, hash) tuples for deterministic comparison
fn get_contract_bindings_hashes(
    element: &element::Element,
    registry: &graph_registry::GraphRegistry,
) -> Vec<(String, String)> {
    let mut hashes: Vec<(String, String)> = element
        ...
        .filter_map(|att| {
            ...
            let hash = match &att.target {
                element::ContractBindingTarget::FilePath(_) => {
                    // File contract_bindings - use stored content_hash
                    att.content_hash.clone()
                }
                element::ContractBindingTarget::ElementIdentifier(elem_id) => {
                    // Element contract_bindings - look up hash from registry
                    registry
                        .get_element(elem_id)
...
/// Get the set of contract_bindings target strings that changed between two versions
fn get_changed_contract_bindings(
    ...
    // Check for removed contract_bindings (hash in reference but not in current)
    ...
    for (target, ref_hash) in &ref_hashes {
        if !cur_hash_set.contains(ref_hash) {
            // Hash was removed - find if there's a current contract_bindings with same target
...
pub fn compute_change_impact(
    ...
    for id in current_ids.intersection(&reference_ids) {
        ...
        let ref_hash_set: HashSet<&String> = ref_contract_bindings_hashes
            ...
            .map(|(_, h)| h)
            .collect();
        let contract_bindings_changed = cur_hash_set != ref_hash_set;
        ...
        // Get changed contract_bindings for rendering
        ...
        if has_changed {
            ...
            log::debug!(
                "Changed element '{}': content={}, contract_bindings={}, relations={}",
                cur_elem.name,
                content_changed,
                contract_bindings_changed,
...
mod tests {
    ...
    fn concept_payload(identifier: &str, name: &str) -> crate::element::Concept {
        crate::element::Concept {
            iri: format!(
                "https://example.test/concepts#{}",
                name.split_whitespace().collect::<String>()
            ),
            pref_label: name.to_string(),
            definition: None,
            scheme_iri: Some("https://example.test/concepts#ReqvireConceptScheme".to_string()),
            labels: Vec::new(),
            scope_note: None,
            examples: Vec::new(),
            broader: Vec::new(),
            ...
            exact_match: Vec::new(),
            close_match: Vec::new(),
            namespace_base: Some("https://example.test/concepts".to_string()),
            namespace_prefix: Some("concept".to_string()),
            source_element_identifier: identifier.to_string(),
        }
    }

    #[test]
    fn concept_change_impacts_ontology_terms_mapped_to_concept() {
        let mut current = GraphRegistry::new();
        let mut reference = GraphRegistry::new();

        let mut current_concept = create_typed_element(
            "Thesaurus.md#verification-coverage",
            "Verification Coverage",
            "Updated concept definition.",
            crate::element::ElementType::Concept,
        );
        current_concept.concept = Some(concept_payload(
            "Thesaurus.md#verification-coverage",
            "Verification Coverage",
        ));

        let mut reference_concept = create_typed_element(
            "Thesaurus.md#verification-coverage",
            "Verification Coverage",
            "Original concept definition.",
            crate::element::ElementType::Concept,
        );
        reference_concept.concept = Some(concept_payload(
            "Thesaurus.md#verification-coverage",
            "Verification Coverage",
        ));

        let ontology_content = r#"#### Ontology
```turtle
@prefix concept: <https://example.test/concepts#> .
@prefix ex: <https://example.test/ontology#> .
@prefix reqvire: <https://www.reqvire.org/ontology#> .

ex:VerificationCoverage reqvire:mapsToConcept concept:VerificationCoverage .
```
"#;
        let ontology = create_typed_element(
            "Ontology.md#verification-ontology",
        ...
        assert!(concept_tree.relations.iter().any(|relation| {
            relation.relation_trigger == "mappedByOntology"
                && relation.element_node.element.name == "Verification Ontology"
        }));
    }
