...
- Model evidence tools shall expose the canonical capability/requirement/ontology model, including `ontology` elements, `#### Concept References`, and requirement-owned `semantic-contract` shape profiles where the underlying Reqvire operation returns them.
