...
- Streamable HTTP transport uses JSON-RPC over HTTP according to the MCP Streamable HTTP transport rules.
...
- The endpoint supports HTTP POST for client JSON-RPC messages.
- The endpoint may support HTTP GET with `Accept: text/event-stream` for SSE streaming.
- If SSE streaming is not implemented, HTTP GET returns method-not-allowed behavior according to MCP transport rules.
- HTTP responses use MCP JSON-RPC response bodies or SSE events as required by the selected request mode.
...
- Requests with a loopback `Origin` header are allowed for local browser-based tools.
- Requests with a non-loopback, `null`, file, or malformed `Origin` header are rejected before executing MCP requests.
