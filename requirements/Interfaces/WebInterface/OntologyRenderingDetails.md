...
#### Details
This document describes how the Ontologies HTML explorer turns authored RDF/Turtle, SHACL, and generated Reqvire ontology projection facts into the rendered graph.
...
- Object, datatype, and RDF properties are not graph boxes. They are projected into labeled relationship semantics, inspector evidence, domain/range rows, SHACL slot rows, badges, and search text.
...
3. `build_graph_data` builds an `ontologyGraphData` JSON model containing semantic nodes, construct edges, property metadata, SHACL slot/facet records, badges, source links, and inspector evidence.
...
The generated `ontologyGraphData` is the contract between Rust projection logic and browser rendering. Filters and layout may hide graph items on the canvas, but they must not remove evidence from the selected node inspector.
...
- RDF list plumbing is suppressed as graph nodes and raw `rdf:first`/`rdf:rest` edges; list members are preserved as readable construct evidence, such as class-expression members in the inspector and property-chain member rows.
- Property details remain available in the inspector for selected classes, individuals, and terms.
...
Property semantic kinds are still visible, but through edge label palettes and inspector badges rather than node fill colors.
...
- Inverse/equivalence/disjoint/property-chain/property-characteristic links: construct-specific edges governed by relation visibility controls when they have a direct canvas-visible control; otherwise they remain passive notation and inspector evidence.
...
- Restriction links remain construct notation around restriction glyph nodes, with `on property` and restriction-kind evidence in the edge label/inspector instead of pretending the restriction is an ordinary property relationship.
...
The `Show` controls expose role and relation visibility toggles in one button group:
- Terms
...
Checked means shown. The default state shows datatype-property links, object-property links, and class-membership edges. Restrictions, class expressions, and SHACL shapes are opt-in so the default graph remains readable.
...
When a node is selected or hovered, the focused neighborhood is computed from currently visible relations. Disabling a relation filter removes neighbors that were reachable only through that relation from the selected/hovered focus highlight, while ordinary term visibility remains controlled by the `Terms` role filter.
...
Equivalence, inverse-property, property-chain, and property-characteristic constructs are currently passive `Notation` legend and inspector evidence. They should not be exposed as active filters until they have a direct canvas-visible effect.

Changing any filter must not reset the camera. This keeps before/after visual comparison possible.

#### Inspector Semantics
The inspector is evidence-oriented and intentionally independent from canvas filters:
...
- Filtered-out badges, constructs, or SHACL evidence must remain available in the inspector when they belong to the selected node.
...
- Click selection persists focus, centers the selected node, and opens the inspector.
...
- Exported `ontologies.ttl` remains authored ontology/SHACL content only.
- The exported Ontologies SPA route includes the Sigma, Graphology, ForceAtlas2, `@sigma/edge-curve`, and `@sigma/node-image` renderer evidence.
...
- Keep properties as edges/inspector evidence unless there is a strong semantic reason to reintroduce them as nodes.
- Keep filters canvas-only and preserve inspector evidence.
