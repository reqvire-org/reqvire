...
| **definedBy** | define | Yes | No | Yes | Links a requirement to subordinate contract elements |
| **define** | definedBy | No | Yes | No | Links a contract element to the requirement it defines |
...
2. **Requirement Changes**:
   ...
   - Changes to requirements propagate to all owned contract artifacts
...
5. **Contract Changes**:
   - Changes to requirements propagate to owned contract artifacts via `definedBy`
   - Contract elements define requirement terms; changes to the requirement may require updating its contracts
...
| **definedBy** | requirement | subtype-specific contract types | Requirements own subordinate contracts |
| **define** | contract types | requirement | Inverse of definedBy (auto-generated), constrained by contract subtype |
...
| **trace** | Any (except contract types) | Any | Documentation/discovery, no type constraints |

### Element-Centric View

| Element Type | Can Use as Source | Can Be Target Of |
|--------------|-------------------|------------------|
| **capability** | derivedFrom, derive, specifiedBy, verifiedBy, trace | derivedFrom, derive, specify, verify, trace |
| **requirement** | derivedFrom, derive, satisfiedBy, definedBy, constrainedBy, verifiedBy, trace | derivedFrom, derive, satisfy, define, constrain, verify, trace |
...
| **source** | define | definedBy |
| **semantic-contract** | constrain, use | constrainedBy, usedBy |
| **constraint** | define | definedBy, Requirement Attachment |
| **behavior** | define | definedBy, Requirement Attachment |
| **specification** | define | definedBy, Requirement Attachment |
| **state** | define | definedBy, Requirement Attachment |
| **input-output** | define | definedBy, Requirement Attachment |
| **other** | trace | trace |
...
3. **Contract ownership excludes semantic contracts**: Elements of type `source`, `constraint`, `behavior`, `specification`, `state`, and `input-output` can only use `define` relations to link to their compatible requirement owner. Each ordinary contract can only be owned by one valid requirement. Semantic contracts use `constrain`/`constrainedBy` and `use`/`usedBy` instead of `define`/`definedBy`.
