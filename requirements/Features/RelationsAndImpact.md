...
SHACL profile split from Reqvire Change Impact Ontology so ontology vocabulary remains first-class and requirement-owned semantic contracts carry closed-world constraints.
...
SHACL profile split from Reqvire Relation Ontology so ontology vocabulary remains first-class and requirement-owned semantic contracts carry closed-world constraints.
...
#### Details
Trace changes in system model is the capability anchor for impact propagation, auditable dependency paths, and review routing after model changes.

Change impact uses native Reqvire relations and explicit attachments. Semantic references are modeled through feature-root hierarchy or attachment context so change impact is not hidden in an unrelated ontology reference.
