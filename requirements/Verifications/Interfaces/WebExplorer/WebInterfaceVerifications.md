...
#### Relations
  * verify: [Attachment Link Serving](../../../Interfaces/WebExplorer/Capabilities.md#attachment-link-serving)
---
...
#### Relations
  * verify: [Diagram Attachment Display](../../../Interfaces/WebExplorer/Capabilities.md#diagram-attachment-display)
---
...
#### Relations
  * verify: [Containment View Attachment Links](../../../Interfaces/WebExplorer/Capabilities.md#containment-view-attachment-links)
---
...
- Assert element-detail modal headers render the actual element type as the only visible type badge; for example a `behavior` element shall not also show a separate `refinement` badge.
...
- Assert the Model tree, grid cards, modeled-element lists, relation/attachment endpoints, and element legends use the shared Explorer `ElementIcon` type glyphs, that capability and semantic-contract elements use their own role colors as plain squares with no glyph, that concept-reference targets use the resource/amber treatment, that evidence-file artifacts use the neutral/default treatment, and that refinement-family subtypes keep the shared refinement color while rendering distinct glyph marks for `source`, `specification`, `constraint`, `behavior`, `state`, and `input-output`.
