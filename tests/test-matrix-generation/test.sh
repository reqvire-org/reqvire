...
OUTPUT=$(cd "$TEST_DIR" && "$REQFLOW_BIN" --config "${TEST_DIR}/reqflow.yaml" --traces 2>&1)
EXIT_CODE=$?
...
JSON_OUTPUT=$(cd "$TEST_DIR" && "$REQFLOW_BIN" --config "${TEST_DIR}/reqflow.yaml" --traces --json 2>&1)
EXIT_CODE=$?
...
SVG_OUTPUT=$(cd "$TEST_DIR" && "$REQFLOW_BIN" --config "${TEST_DIR}/reqflow.yaml" --traces --svg 2>&1)
EXIT_CODE=$?
...
CONFLICT_OUTPUT=$(cd "$TEST_DIR" && "$REQFLOW_BIN" --config "${TEST_DIR}/reqflow.yaml" --traces --json --svg 2>&1)
EXIT_CODE=$?
