...
# Setup
TEST_NAME="Excluded Patterns Functionality"
TEST_DIR="../fixtures/test-excluded-patterns"
...
# Run validation with extra verbosity to get detailed logs
echo "Running validation command..."
OUTPUT=$(cd "$TEST_DIR" && "$REQFLOW_BIN" . --validate-all --verbose 2>&1)
EXIT_CODE=$?
...
echo "SUCCESS: Excluded patterns functionality works as expected"
echo "- Relations TO excluded files are correctly validated"
echo "- Elements within excluded files are not processed"
echo ""
echo "Output: $OUTPUT"
