...
# Enhanced JSON structure verification - Test new capabilities
...
# Enhanced content and counts verification - Test new capabilities
...
# 4) --filter-type=capability
echo "Running: reqvire search --json --filter-type=capability" >> "${TEST_DIR}/test_results.log"
set +e
OUTPUT=$(cd "$TEST_DIR" && "$REQVIRE_BIN" search --json --filter-type="capability" 2>&1)
EXIT_CODE=$?
set -e
...
if [ "$(echo "$OUTPUT" | jq '.global_counters.total_elements')" -ne 1 ]; then
  echo "FAILED: --filter-type=capability JSON should yield 1 element"
  exit 1
fi
echo "Running: reqvire search --filter-type=capability" >> "${TEST_DIR}/test_results.log"
set +e
OUTPUT=$(cd "$TEST_DIR" && "$REQVIRE_BIN"  search --filter-type="capability" 2>&1)
EXIT_CODE=$?
set -e
...
if [ "$(grep -c '🔹 Element:' <<< "$OUTPUT")" -ne 1 ]; then
  echo "FAILED: --filter-type=capability text should yield 1 element"
  exit 1
fi
...
echo "Running: reqvire search --json --filter-type=requirement (filter test)" >> "${TEST_DIR}/test_results_requirement_filter.log"
set +e
OUTPUT_REQUIREMENT_FILTER=$(cd "$TEST_DIR" && "$REQVIRE_BIN" search --json --filter-type="requirement" 2>&1)
EXIT_CODE=$?
set -e
