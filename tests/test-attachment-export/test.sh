...
# The attachment should appear in the element node label
if grep -q "DesignSpec" "$SPEC_HTML" && grep -q "mermaid" "$SPEC_HTML"; then
  # Check if diagram has the attachment in the node label (multiline with <br/>)
  if grep -q '📎' "$SPEC_HTML"; then
    : # Success - diagram shows attachment
  else
    # Attachment may not be in diagram yet, but should be in attachments section
    if ! grep -q 'Attachments' "$SPEC_HTML"; then
      echo "❌ FAILED: No Attachments section found in HTML"
      exit 1
    fi
  fi
fi
