...
echo "Running: reqvire search --json (from root)" >> "${TEST_DIR}/test_results.log"
set +e
OUTPUT_ROOT=$(cd "${TEST_DIR}" && "$REQVIRE_BIN" search --json 2>&1)
EXIT_CODE_ROOT=$?
set -e
...
echo "Running: reqvire search --json (from subfolder)" >> "${TEST_DIR}/test_results.log"
set +e
OUTPUT_SUB=$(cd "${TEST_DIR}/specifications/subfolder" && "$REQVIRE_BIN" search --json 2>&1)
EXIT_CODE_SUB=$?
set -e
...
# Subfolder should process the local capability plus its two requirements.
