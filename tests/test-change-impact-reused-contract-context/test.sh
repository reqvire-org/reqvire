...
# Test: Reused Contract Context Change Impact
# --------------------------------------
# Satisfies: specifications/System/Core/Verifications/ReusedContractContextVerifications.md#reused-contract-context-change-impact-verification
#
# Tests:
# 1. Contract content change is detected (with propagation to reused requirements)
# 2. File reused_contract_context content change is detected (propagates to reused requirement)
# 3. mv Contract to different file - relocation reported, identifiers updated
# 4. rm Contract element - validation fails

echo "===================================="
echo "Reused Contract Context Change Impact Tests"
echo "===================================="
echo ""
...
# ==================================
echo "Test 2: Second reused contract content change is detected..."

# Modify second reused contract content
...
# Compare against expected output
if ! diff -u "${TEST_SCRIPT_DIR}/expected/expected-file-reused-contract-context-change.txt" <(echo "$SANITIZED_OUTPUT"); then
  echo "❌ FAILED: Second reused contract content change output does not match expected."
  echo ""
  echo "If changes are intentional, update ${TEST_SCRIPT_DIR}/expected/expected-file-reused-contract-context-change.txt"
  exit 1
fi

# Commit the change for next test
cd "$TEST_DIR" && git add -A && git commit -q -m "Change reused contract content"
...
# Verify reused_contract_context identifiers were updated in Requirements.md
if ! grep -q "Contracts.md#data-format-spec" "$TEST_DIR/specifications/Requirements.md"; then
  echo "FAILED: ReusedContractContextEntry identifiers should be updated after mv"
...
# ==================================
echo "Test 4: rm Contract element with reused_contract_context - validation fails..."

# Remove Data Format Spec (reused to 3 requirements)
set +e
RM_OUTPUT=$(cd "$TEST_DIR" && "$REQVIRE_BIN" rm "Data Format Spec" 2>&1)
RM_EXIT=$?
set -e

# Validation should now fail because reused_contract_context are broken
set +e
...
if [ $VALIDATE_EXIT -eq 0 ]; then
  echo "FAILED: Validation should fail when Contract with reused_contract_context is removed"
  echo "$VALIDATE_OUTPUT"
  exit 1
fi

# Verify error mentions the broken reused_contract_context
if ! echo "$VALIDATE_OUTPUT" | grep -qi "data-format-spec\|not found\|invalid\|missing"; then
  echo "FAILED: Validation error should mention the broken reused_contract_context identifier"
  echo "$VALIDATE_OUTPUT"
  exit 1
fi
...
# ==================================
echo "===================================="
echo "All Reused Contract Context Change Impact tests passed"
echo "===================================="
exit 0
