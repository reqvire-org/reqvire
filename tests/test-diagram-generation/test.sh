...
for file in "${FILES_TO_CHECK[@]}"; do
  ...
  # Special handling for VerificationTests.md - should NOT be modified (custom diagram preserved)
  if [ "$file" == "VerificationTests.md" ]; then
    if ! cmp -s "$file_path" "$backup_path"; then
      echo "❌ MODIFIED: Expected VerificationTests.md to be preserved (custom diagram without marker): $file"
      FAILED_CHECKS=$((FAILED_CHECKS + 1))
    fi
    # Still verify it has mermaid content
    if ! grep -q '```mermaid' "$file_path"; then
      echo "❌ NO DIAGRAM: Expected file to contain mermaid diagram: $file"
      FAILED_CHECKS=$((FAILED_CHECKS + 1))
    fi
    # Verify it does NOT have the marker (it's a custom diagram)
    if grep -q 'REQVIRE-AUTOGENERATED-DIAGRAM' "$file_path"; then
      echo "❌ UNEXPECTED MARKER: Custom diagram should not contain REQVIRE-AUTOGENERATED-DIAGRAM marker in $file"
      FAILED_CHECKS=$((FAILED_CHECKS + 1))
    fi
    continue
  fi
