...
export interface OntologyDetailTerm {
  label?: string;
  iri?: string;
  kind?: string;
}

export interface OntologyDetailSource {
  source?: string;
  source_name?: string;
  file_path?: string;
  line_number?: number;
...
export interface OntologyDetailSlotFacet {
  slot_label?: string;
  slot_iri?: string;
  slot_kind?: string;
  ...
  source?: OntologyDetailSource;
  facets?: { name?: string; value?: string }[];
}

export interface OntologyDetailConstruct {
  id: string;
  family?: string;
  kind?: string;
  label?: string;
...
export interface OntologyDetailNode {
  id: string;
  label?: string;
  type?: string;
  node_type?: string;
  ...
  domain?: OntologyDetailTerm[];
  range?: OntologyDetailTerm[];
...
export interface OntologyNodeDetailDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  node?: OntologyDetailNode;
  nodes?: OntologyDetailNode[];
  missingNodeId?: string | null;
  copiedUri: boolean;
  onCopyUri: (uri: string) => void;
  onOpenSource: (source: OntologyDetailSource) => void;
}

export interface OntologyNodeDetailContentProps {
  node: OntologyDetailNode;
  nodes: OntologyDetailNode[];
  copiedUri: boolean;
  onCopyUri: (uri: string) => void;
...
export interface OntologyMetadataProps {
  node: OntologyDetailNode;
  copiedUri: boolean;
  onCopyUri: (uri: string) => void;
}

export interface OntologyTermListProps {
  terms: OntologyDetailTerm[];
  emptyLabel: string;
}

export interface OntologyPropertyUsagesProps {
  node: OntologyDetailNode;
  nodes: OntologyDetailNode[];
}

export interface OntologyConstructRowsProps {
  constructs: OntologyDetailConstruct[];
}

export interface OntologySourceListProps {
  sources: OntologyDetailSource[];
  onOpenSource: (source: OntologyDetailSource) => void;
}

const detailDialogBaseUX = css`
  --ux-ontology-detail-dialog-w: 1120px;
  --ux-ontology-detail-dialog-max-h: 980px;
  --ux-ontology-detail-dialog-body-max-h: 780px;
...
const ontologyDialogBaseUX = css`
  --ux-ontology-dialog-w-md: 1040px;
  --ux-ontology-dialog-w-sm: 760px;
  width: min(var(--ux-ontology-dialog-w-md), calc(100vw - var(--space-24)));

  @media (max-width: 980px) {
    width: min(var(--ux-ontology-dialog-w-sm), calc(100vw - var(--space-14)));
  }
`;
...
const detailBodyBaseUX = css`
  max-height: min(74vh, var(--ux-ontology-detail-dialog-body-max-h));
  overflow: auto;
  padding: var(--space-14) var(--space-16);

  @media (max-width: 720px) {
    max-height: calc(100vh - var(--ux-ontology-detail-dialog-chrome-h));
    padding: var(--space-8);
  }
`;
...
const metadataRowBaseUX = css`
  display: grid;
  grid-template-columns: var(--ux-ontology-meta-key-col) minmax(0, 1fr);
  gap: var(--space-8);
...
const ontologyUriCopyBaseUX = css`
  ...
  cursor: pointer;
  --ds-coderef-min-w: 0;
  --ds-coderef-ow: anywhere;
  --ds-coderef-text-align: left;
  --ds-coderef-ws: normal;
...
const ontologyConstructGlyphSkinX = css`
  border: var(--border-w) solid var(--rdf-class-border);
  border-radius: var(--radius-pill);
  background: var(--rdf-class-tint);
  color: var(--text-strong);
`;
...
const ontologyMetaRowUX = css`
  display: grid;
  grid-template-columns: var(--ux-ontology-meta-key-col-narrow) minmax(0, 1fr);
  gap: var(--space-7);
...
function OntologyDetailDialogFrame({
  open,
  onOpenChange,
  children,
  contentClassName,
}: OntologyDetailDialogFrameProps) {
  return (
    <Modal open={open} onOpenChange={onOpenChange}>
      <ModalContent
        className={cx(detailDialogBaseUX, detailDialogSkinX, contentClassName)}
...
export function OntologyNodeDetailDialog({
  ...
  const primarySource = node?.sources?.find((source) => source.link) ?? node?.sources?.[0];
  return (
    <OntologyDetailDialogFrame open={open} onOpenChange={onOpenChange} contentClassName={cx(ontologyDialogBaseUX)}>
      {!node ? (
        <>
          ...
          <ModalBody className={cx(detailBodyBaseUX, detailBodySkinX, ontologyBodyUX)}>
            <p className={cx(detailMutedUX)}>
              No exported ontology graph node matches <CodeRef>{missingNodeId ?? ""}</CodeRef>.
            </p>
          </ModalBody>
        </>
      ) : (
    ...
    </OntologyDetailDialogFrame>
  );
}

export function OntologyNodeDetailContent({
  node,
  nodes,
  copiedUri,
  onCopyUri,
      ...
      <main className={cx(ontologyColumnUX)}>
        ...
        {node.constructs?.length ? (
          <Section title="Projection constructs">
            <OntologyConstructRows constructs={visibleConstructs(node)} />
          </Section>
        ) : null}
          ...
          <Section title="Literal values">
            <div className={cx(ontologyGridListUX)}>
              {node.literal_values.map((literal, index) => (
                <div
                  className={cx(ontologyCardBaseUX, ontologyCardSkinX, ontologyCardCompactUX)}
                  key={`${literal.predicate}-${index}`}
                >
          ...
          <Section title="Raw SHACL evidence">
            <div className={cx(ontologyGridListUX)}>
              {node.constraints.map((constraint) => (
                <div
                  className={cx(ontologyCardBaseUX, ontologyCardSkinX, ontologyCardCompactUX)}
                  key={`${constraint.property}-${constraint.value}`}
                >
        ...
        {node.sources?.length ? (
          <Section title="Sources">
            <OntologySourceList sources={node.sources} onOpenSource={onOpenSource} />
          </Section>
        ) : null}
      </main>
    </div>
...
export function OntologyMetadata({
  node,
  copiedUri,
  onCopyUri,
    ...
    <div className={cx(metadataBaseUX, metadataSkinX)}>
      ...
      <div className={cx(metadataRowBaseUX, metadataRowSkinX)}>
        ...
        <span className={cx(metadataValueBaseUX, metadataValueSkinX)}>
          <button
            type="button"
            className={cx(ontologyUriCopyBaseUX, ontologyUriCopySkinX)}
            onClick={() => onCopyUri(uri)}
            title="Copy URI"
          >
            <CodeRef>{uri}</CodeRef>
...
export function OntologyPropertyUsages({
  node,
  nodes,
          ...
          <div className={cx(ontologyCardBaseUX, ontologyCardSkinX)} key={`${usage.property.id}-${usage.role}`}>
            ...
            <MetaRows
              rows={[
                ...
                { key: "domain", value: <OntologyTermList terms={usage.domains} emptyLabel="Any" /> },
                { key: "range", value: <OntologyTermList terms={usage.ranges} emptyLabel="Any" /> },
              ]}
            />
            {usage.facets.length ? (
              <div className={cx(ontologyInlineListUX)}>
...
export function OntologyConstructRows({ constructs }: OntologyConstructRowsProps) {
  return (
    <div className={cx(ontologyGridListUX)}>
...
export function OntologySourceList({ sources, onOpenSource }: OntologySourceListProps) {
  return (
    <div className={cx(ontologyGridListUX)}>
      {dedupeSources(sources).map((source, index) => (
        <SourceRow key={`${source.link}-${index}`} source={source} onOpenSource={onOpenSource} />
      ))}
    </div>
  );
}

export function OntologyTermList({
  terms,
  emptyLabel,
...
function OntologyNodeModalHeader({ node }: { node: OntologyDetailNode }) {
...
function ConstructRow({ construct }: { construct: OntologyDetailConstruct }) {
...
function dedupeSources(sources: OntologyDetailSource[]): OntologyDetailSource[] {
  const byLocation = new Map<string, OntologyDetailSource>();
...
function propertyUsagesForNode(node: OntologyDetailNode, nodes: OntologyDetailNode[]) {
  return nodes
...
function OntologyTerm({ term }: { term: OntologyDetailTerm }) {
  return (
...
function visibleConstructs(node: OntologyDetailNode) {
...
function constructGlyph(construct: OntologyDetailConstruct) {
