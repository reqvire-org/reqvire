...
A design system for **Reqvire Explorer**, the interactive viewer for [Reqvire](https://www.reqvire.org/) system models. Reqvire is a Git-native MBSE (model-based systems engineering) toolkit: engineering knowledge — capabilities, requirements, refinements, semantic contracts, verifications and traceability links — lives as semi-structured Markdown in a repository, and Reqvire compiles it into a typed knowledge graph with auto-generated diagrams and reports.
...
| **Traces** | Sankey flow of files → requirements → verifications (traceability coverage). |
...
- **Identifiers and paths are monospace**, always with a line anchor when relevant: `requirements/Capabilities.md:3`, `requirements/Capabilities.md#defining-model-structure`.
...
**Backgrounds & imagery.** No photography, no illustration. The "imagery" of the product *is* the data: node clouds, sankey ribbons, RDF graphs. Keep backgrounds flat; let the colored nodes carry the visual energy.
