...
impl<'a> ModelDiagramGenerator<'a> {
    ...
    pub fn generate_mermaid(&self, report: &ModelDiagramReport) -> String {
        ...
        diagram.push_str("  classDef designDocument fill:#E3F2FD,stroke:#2196F3,stroke-width:1.5px;\n");
        ...
        for folder in &report.folders {
            ...
            for file in &folder.files {
                ...
                for element in &file.elements {
                    ...
                    // Build label with element name only (attachments shown separately)
                ...
                // Add Attachments subgraph if any element has attachments
                let all_attachments: Vec<(&ElementNode, &String)> = file.elements.iter()
                    .flat_map(|e| e.attachments.iter().map(move |a| (e, a)))
                    .collect();

                if !all_attachments.is_empty() {
                    let attachments_id = utils::hash_identifier(&format!("{}_attachments", file.path));
                    diagram.push_str(&format!("    subgraph {}[\"📎 Attachments\"]\n", attachments_id));

                    for (element, attachment) in &all_attachments {
                        let attachment_id = utils::hash_identifier(&format!("{}_{}", element.identifier, attachment));
                        diagram.push_str(&format!("      {}[\"📝 {}\"];\n", attachment_id, escape_label(attachment)));
                        diagram.push_str(&format!("      class {} designDocument;\n", attachment_id));
                    }
...
fn add_element_to_diagram(
    ...
    if !included_elements.contains(&element.identifier) {
       ...
       eprintln!("DEBUG: Element '{}' has {} attachments", element.name, element.attachments.len());
...
pub fn generate_containment_diagram(registry: &GraphRegistry, short: bool) -> Result<String, ReqvireError> {
    ...
    output.push_str("  classDef designDocument fill:#E3F2FD,stroke:#2196F3,stroke-width:1.5px;\n\n");
...
fn generate_folder_tree(
    ...
    for doc in &folder.design_documents {
        ...
        output.push_str(&format!("  {}[\"📝 {}\"]\n", doc_id, doc.name));
        output.push_str(&format!("  {} --> {}\n", parent_id, doc_id));
        output.push_str(&format!("  class {} designDocument\n", doc_id));
    }
