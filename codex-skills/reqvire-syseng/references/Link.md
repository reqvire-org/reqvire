...
Create or remove relations between elements, or reuse/unreuse files and contract elements.

---

## Linking Elements

Create a relation between elements or reuse files/contract elements. This handles both relations and reused_contract_context.
...
- **Duplicate detection**: The link command fails if the relation or reused_contract_context already exists
- **Cross-section duplicates**: Cannot add a relation to a target that already exists as a reused_contract_context (and vice versa)
...
2. **Then**: If no relation found, searches for a reused_contract_context matching the target
...
**Remove a reused_contract_context file:**
```bash
