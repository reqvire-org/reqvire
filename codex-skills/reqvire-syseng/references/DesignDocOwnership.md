...
- Some reusable contract elements are referenced only as Contract Bindings and need an explicit owning requirement
- You need one owning requirement per contract element

## Goal

Assign each contract element to a single owning requirement via `definedBy` (identifier target), place it in the same directory as that requirement, and keep other consumers as contract_bindings when reuse is intentional.
...
- Exceptions that should remain shared Contract Bindings without becoming locally owned contracts
...
4. Keep all non-owner references as contract_bindings
...
- Do not blindly replace every contract_bindings with `definedBy`
