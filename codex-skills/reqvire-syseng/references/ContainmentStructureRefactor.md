...
In this reference, `./` means the chosen Reqvire model root relative to the effective workspace root where Reqvire is run. That model root may be the workspace root, `system-model/`, `specs/system-model/`, or another project-selected folder. Do not assume the whole Git repository root is the model root.
