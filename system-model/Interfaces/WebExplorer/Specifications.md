...
The Model project tree must initialize with the visible top-level `Model` and `Resources` branches and their Git worktree identity folders expanded. First useful content folders and files under each worktree must be visible, but deeper child folders and file element rows must remain collapsed until the user expands them or a later explicit selection workflow requires revealing a selected descendant.

The Model tree must not spend a visible row on the workspace root itself. Its visible top-level rows must be `Model` and `Resources`. Repository, branch, and eligible Git worktree details remain available as Project Store metadata; eligible Git worktree identity must appear as grouping folders under both top-level branches so single-repo workspaces still show the repository name and multi-repo workspaces show each repository/worktree separately.
...
- Seed source-file content directly from the in-memory graph registry: files with modeled elements use normalized registry-generated Markdown.
- Keep implementation files, evidence files, scripts, images, documentation files, and other local relation targets as Project Store `resources` for relation semantics. Existing eligible Git-worktree resource targets may carry captured raw source preview content when available, but resource-only paths must not be projected into Project Store `files` or `folders`.
- Expose Project Store `resources` through Explorer resource/evidence navigation, including a top-level `Resources` branch beside the top-level `Model` branch in the Model tree. The `Resources` branch must preserve workspace-root-relative folder structure for local resource paths without treating those paths as modeled source-file containers.
- Build the Project Store `files` and `folders` sections only from modeled element source files in the graph registry. Resource-only local targets, unrelated Git-worktree files, non-Git workspace files, unsupported parsed pages, nonexistent local targets, and external URLs must not become Explorer file-tree or file-search records.
- Preserve full workspace-root-relative hierarchy for every Project Store file path so the Model tree renders modeled folders, modeled files, and file-owned elements without flattening single-element files.
...
- Preserves interactive graph manipulation while filtering, focusing, panning, zooming, dragging nodes, and resizing; filtering must not reset the current camera, focusing must center the selected node through Sigma's display-coordinate mapping, and reset must rerun the ForceAtlas2 layout, refresh the stable layout baseline, and fit the graph without losing ontology-diagram visual conventions. The viewer exposes a single visible `Reset` view control rather than separate fit and reset buttons.
- When an ontology node is selected, computes the currently visible ontology focus tree, runs bounded Graphology no-overlap layout only for that focus tree from stable post-ForceAtlas baseline coordinates, keeps the selected node anchored, scales no-overlap spacing with focused-neighborhood size, centers the camera through Sigma's display-coordinate mapping after the selected layout target is applied, restores every graph node outside the current focus tree to the stable baseline, restores the full graph to that baseline when selection is cleared or hidden by filters, and animates affected coordinates through Sigma node animation without rerunning the full graph layout or accumulating coordinate drift.
...
- Centers the graph viewport on a node selected from search or direct graph click through Sigma's display-coordinate mapping while preserving the current zoom ratio; filter-only changes must not recenter the viewport.
...
- Uses Graphology ForceAtlas2 over the currently visible rendered project graph with Barnes-Hut optimization, deterministic initial positions, and data-dependent settings derived from visible node count, visible edge density, and average rendered node size. Hidden-by-filter nodes and hidden overlay edges must not contribute repulsion or spacing to the visible full-graph baseline, and the renderer must avoid fixed repulsion constants that make large sparse graphs form hollow ring layouts. The Model Graph route must paint its shell and graph-canvas loading notice with the design-system spinner glyph before Sigma/ForceAtlas construction is scheduled in a post-paint task. Interaction reducers must use precomputed adjacency and cached focus neighborhoods so hover and pinned-selection behavior does not repeatedly scan the full edge list.
- When a node is clicked, the Model Graph must compute the currently visible direct neighborhood, run bounded Graphology no-overlap layout only for that focused neighborhood from stable post-ForceAtlas baseline coordinates, keep the selected node anchored, scale no-overlap spacing with focused-neighborhood size, center the camera through Sigma's display-coordinate mapping after the selected layout target is applied, restore nodes from the previous focused neighborhood when they leave focus through filter changes, selection changes, or selection clearing, and animate the affected node coordinates through Sigma node animation. This selected-neighborhood layout must not rebuild the Graphology graph, rerun the full ForceAtlas layout, move unrelated hidden nodes, recenter on filter-only changes, or accumulate coordinate drift across repeated selections.
...
- `files`: modeled source/document file containers keyed by workspace-root-relative path, with display path, source route path, parent folder, child element ids, local asset links, and containment metadata.
...
- `files` are browser-local modeled source containers. They represent Markdown/source documents that contain model elements and are used for containment, file navigation, source links, and breadcrumbs.
...
- A path may appear as a `file` only when it contains modeled elements in the graph registry. A path appears as a `resource` when it is referenced as evidence or a modeled target by relation, contract_bindings, or resource-report facts.
...
- The virtual filesystem root is an internal container only; it must not render as a visible Model tree row. Source-control details such as repository, branch, Git `HEAD`, dirty state, and eligible worktree roots are metadata used to label worktree grouping folders.
- The left Model tree must render virtual `Model` and `Resources` branches as visible top-level rows. Under each branch, eligible Git worktree folders group the corresponding entries. The virtual `Model` branch contains the modeled source file hierarchy using real workspace-root-relative paths under each worktree grouping folder; the virtual `Resources` branch contains resource/evidence hierarchy using real workspace-root-relative paths for local targets under each worktree grouping folder.
- Folder/file containment is modeled source organization only; it does not define logical capability, requirement, ontology, or verification ownership.
...
- Resource navigation must use `resources` records and referring facts; it must not imply that evidence files contain model elements. Resource-only paths must not appear in the modeled source file hierarchy, but graph-linked resources must remain visible as resource/evidence nodes with folder structure when a local path exists.
