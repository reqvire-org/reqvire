...
- Render model element concept references inline in element-detail modal prose when the referenced native SKOS concept preferred label, alternative label, or authored reference label appears in the content. Inline concept-reference links use the standard Explorer link color, no resting background, no glyph, no badge, no pill, and show underline only on hover or focus. Activating the link opens the native concept element modal. The authored `#### Concept References` source subsection must be hidden from regular element modal content rendering. Concept-reference links must not open ontology-node modals. Evidence-file and artifact targets use the neutral/default role treatment and must not share the concept-reference color.
...
- Element-detail modal concept references must resolve to native `concept` elements through Project Store `target_element_id`, render matching prose terms as inline concept links, keep the derived SKOS concept IRI as tooltip/location metadata, and open the native concept element modal. They must not route through ontology-node fallback behavior.
