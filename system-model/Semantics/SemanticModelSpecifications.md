...
The runtime Reqvire ontology artifact contract defines the generated Turtle snapshot embedded by Reqvire core.
...
- The embedded constant exposes the generated Turtle content without requiring runtime access to `system-model/Ontologies`.
...
- The artifact must include generated ontology document declarations, generated term definition links, authored runtime Reqvire ontology RDF, and authored runtime semantic-contract SHACL RDF whose declared subjects are in the runtime Reqvire term namespace.
...
- The artifact must not include ontology/SHACL blocks whose declared subjects are outside the runtime Reqvire term namespace.
...
The synchronization contract defines how Reqvire prevents the embedded runtime ontology artifact from drifting from the authored ontology model.
...
- The regenerated and curated temporary output shall be compared with `crates/reqvire-core/src/runtime_ontology/reqvire.ttl` after deterministic blank-node label normalization.
...
```bash
cargo run -q -p reqvire-cli -- ontologies --namespace-base https://www.reqvire.org/ontology# --output crates/reqvire-core/src/runtime_ontology/reqvire.ttl
```

If the authored model includes concept-layer imports or bridge usages that are not runtime bootstrap facts, maintainers must apply the documented runtime curation before committing `reqvire.ttl`. The curation is part of the runtime artifact contract, not an ad hoc edit.
