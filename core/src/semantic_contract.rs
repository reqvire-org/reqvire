use crate::element::{
    self, ContractType, Element, ElementType, FencedBlock, ReusedContractContextTarget,
...
impl SemanticIndex {
    ...
    pub fn to_jsonld_string_with_external(
        ...
        let ontology_documents_turtle =
            build_ontology_document_declarations_turtle(&self.ontology_documents);
        if !ontology_documents_turtle.trim().is_empty() {
            for parsed in RdfParser::from_format(RdfFormat::Turtle)
                .for_reader(ontology_documents_turtle.as_bytes())
            {
                let quad = parsed.map_err(|error| {
                    ReqvireError::SerializationError(format!(
                        "Ontology document declarations failed to parse: {}",
                        error
                    ))
                })?;
                serializer
...
fn normalized_relation_projection(relation_name: &str) -> Option<NormalizedRelationProjection> {
    let projection = match relation_name {
        ...
        "definedBy" => NormalizedRelationProjection {
            forward_property: "requirementDefinesContract",
            inverse_property: "contractDefinedByRequirement",
            direction: RelationProjectionDirection::Forward,
        },
        ...
        "verifiedBy" => NormalizedRelationProjection {
            forward_property: "elementVerifiedByVerification",
            inverse_property: "verificationVerifiesElement",
            direction: RelationProjectionDirection::Forward,
        },
...
fn extract_external_ontology_sources(
    ...
    for (line_index, line) in element.content.lines().enumerate() {
        let trimmed = line.trim();
...
fn finish_external_ontology_section(
    element: &Element,
    sources: &mut Vec<ExternalOntologySource>,
    ...
    {
        sources.push(ExternalOntologySource {
...
fn build_external_ontology_block(
    ...
    Some(SemanticBlock {
        ...
        source: format!("{}#external-ontology-{}", element.identifier, source.prefix),
...
fn validate_semantic_sections(
    ...
    let has_external_ontology_section =
        element::has_subsection(&element.content, "External Ontology");
...
fn build_ontology_projection(
    ...
    let mut builder = OntologyProjectionBuilder::new(rdf_lists, object_index);
...
fn ontology_term_declarations_from_quads(
    ...
    ontology_term_declarations_from_quads_with_source(element, quads, false)
}
...
fn external_ontology_term_declarations_from_quads(
    ...
    ontology_term_declarations_from_quads_with_source(element, quads, true)
}
