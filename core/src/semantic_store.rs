...
pub const GRAPH_SHACL: &str = "urn:reqvire:semantic-graph:shacl";
pub const GRAPH_MODEL_CONTEXT: &str = "urn:reqvire:semantic-graph:model-context";
pub const GRAPH_ONTOLOGY_PROJECTION: &str = "urn:reqvire:semantic-graph:ontology-projection";
pub const GRAPH_RAW_EXTERNAL_SOURCE: &str = "urn:reqvire:semantic-graph:raw-external-source";
...
impl SemanticModelStore {
    pub fn build(registry: &GraphRegistry) -> Result<Self, ReqvireError> {
        ...
        let turtle_parts = SemanticStoreTurtleParts::from_index(&index)?;
...
struct SemanticStoreTurtleParts {
    ...
    ontology_projection: String,
    raw_external_source: String,
    external_used_subset: String,
}

impl SemanticStoreTurtleParts {
    fn from_index(index: &SemanticIndex) -> Result<Self, ReqvireError> {
        Ok(Self {
            authored_ontology: index.to_authored_ontology_turtle_string()?,
            shacl: index.to_shacl_turtle_string()?,
            model_context: index.model_context_turtle.clone(),
            ontology_projection: index.to_ontology_projection_turtle_string(),
...
fn build_public_store(
    ...
    load_public_role(&store, &parts.shacl, GRAPH_SHACL, "SHACL graph")?;
    ...
    if full {
        ...
        load_public_role(
            &store,
            &parts.ontology_projection,
            GRAPH_ONTOLOGY_PROJECTION,
            "ontology projection graph",
        )?;
    }
...
mod tests {
    ...
    fn parts_with_raw_external() -> SemanticStoreTurtleParts {
        SemanticStoreTurtleParts {
            ...
            shacl: "<https://example.test/shape> <https://example.test/p> <https://example.test/o> .".to_string(),
            model_context: "<https://example.test/context> <https://example.test/p> <https://example.test/o> .".to_string(),
            ontology_projection: "<https://example.test/projection> <https://example.test/p> <https://example.test/o> .".to_string(),
