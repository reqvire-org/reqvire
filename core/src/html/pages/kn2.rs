...
(function() {
  ...
  function resolveKn2GraphSource() {
    try {
      ...
      if (projection && Array.isArray(projection.nodes) && Array.isArray(projection.edges)) {
        log('using central project store knowledge_graph projection');
        return projection;
      }
    } catch (error) {
      warn('central store projection unavailable; using page-local kn2GraphData', error);
    }
    return kn2GraphData;
  }
  ...
  const sourceGraph = resolveKn2GraphSource();
  const data = {
    ...
    return [
      ...
      },
      {
        ...
        style: {
          ...
          'font-size': 7,
          'color': '#334155',
          'text-background-color': reqvireSurfaceBase,
