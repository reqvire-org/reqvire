...
Use this prompt when the user asks which requirements or capabilities are verified, unverified, or verified under a specific capability or interface scope.
...
SELECT ?requirement ?verification WHERE {
  ...
  ?requirement reqvire:elementVerifiedByVerification ?verification .
}
```
