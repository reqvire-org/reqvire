...
pub fn generate_model_diagram(registry: &GraphRegistry) -> Result<String, ReqvireError> {
    ...
    let mut folders: Vec<&String> = files_by_folder.keys().collect();
    folders.sort();

    for folder in folders {
        let folder_name = if folder.is_empty() { "root" } else { folder };
        let folder_id = utils::hash_identifier(folder);
        let files = &files_by_folder[folder];

        // Create folder subgraph
        diagram.push_str(&format!("  subgraph {}[\"📁 {}\"]\n", folder_id, escape_label(folder_name)));
        ...
        for file_path in &sorted_files {
            ...
            let file_id = utils::hash_identifier(file_path.as_str());

            // Create file subgraph
            diagram.push_str(&format!("    subgraph {}[\"📄 {}\"]\n", file_id, escape_label(file_name)));

            if let Some(elements) = elements_by_file.get(file_path.as_str()) {
                // Group by section
                ...
                for section_name in sorted_sections {
                    ...
                    let section_id = utils::hash_identifier(&format!("{}::{}", file_path, section_name));

                    // Create section subgraph
                    diagram.push_str(&format!("      subgraph {}[\"§ {}\"]\n", section_id, escape_label(section_name)));
    ...
    for element in registry.get_all_elements() {
        ...
        for relation in &element.relations {
            // Only render relations that should be shown in diagrams
            if !relation::DIAGRAM_RELATIONS.contains(&relation.relation_type.name) {
                continue;
            }
