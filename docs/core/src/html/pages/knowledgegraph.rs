...
const KNOWLEDGE_GRAPH_CSS: &str = r#"
...
.kg-overlay-toggle.is-active {
  border-color: var(--reqvire-surface-border, #c7c1b7);
...
const KNOWLEDGE_GRAPH_JS: &str = r#"
...
const data = {
  nodes: knowledgeGraphData.nodes.map(d => ({ ...d, node_type: d.node_type || d.type || 'other' })),
  edges: knowledgeGraphData.edges.map(d => ({ ...d })),
  submodels: knowledgeGraphData.submodels || []
};
