...
for file in "${FILES_TO_CHECK[@]}"; do
  ...
  # VerificationTests.md - verify it has mermaid content and marker after generation
  # Note: With section removal, there's 1 diagram per file, so diagram generation will
  # add a marked diagram to this file
  if [ "$file" == "VerificationTests.md" ]; then
    # Verify it has mermaid content
    if ! grep -q '```mermaid' "$file_path"; then
      echo "❌ NO DIAGRAM: Expected file to contain mermaid diagram: $file"
      FAILED_CHECKS=$((FAILED_CHECKS + 1))
    fi
    # Verify it has the auto-generated marker (diagram generation adds it)
    if ! grep -q 'REQVIRE-AUTOGENERATED-DIAGRAM' "$file_path"; then
      echo "❌ MISSING MARKER: Expected auto-generated diagram marker in $file"
      FAILED_CHECKS=$((FAILED_CHECKS + 1))
    fi
    continue
  fi

  # Check that other files were modified
  ...
  case "$file" in
    "Requirements.md")
      ...
      ;;
    "Requirements-with-custom-mermaid.md")
      # For the custom mermaid file, ensure all three custom diagrams are preserved
      # Case 1: Section with no elements
      if ! grep -q 'flowchart TD' "$file_path"; then
        echo "❌ LOST CONTENT: Custom flowchart TD diagram not preserved in $file"
        FAILED_CHECKS=$((FAILED_CHECKS + 1))
      fi
      if ! grep -q 'A\[Start\] --> B{Is it?}' "$file_path"; then
        echo "❌ LOST CONTENT: Custom flowchart TD content not preserved in $file"
        FAILED_CHECKS=$((FAILED_CHECKS + 1))
      fi
      # Case 2: Section with elements - custom diagram in body text should be preserved
      if ! grep -q 'flowchart LR' "$file_path"; then
        echo "❌ LOST CONTENT: Custom flowchart LR diagram not preserved in $file"
        FAILED_CHECKS=$((FAILED_CHECKS + 1))
      fi
      if ! grep -q 'X\[Custom Start\] --> Y\[Custom End\]' "$file_path"; then
        echo "❌ LOST CONTENT: Custom flowchart LR content not preserved in $file"
        FAILED_CHECKS=$((FAILED_CHECKS + 1))
      fi
      # Case 3: Section with elements - custom diagram immediately after header should be preserved
      if ! grep -q 'flowchart TB' "$file_path"; then
        echo "❌ LOST CONTENT: Custom flowchart TB diagram not preserved in $file"
        FAILED_CHECKS=$((FAILED_CHECKS + 1))
      fi
      if ! grep -q 'P\[Custom Process\] --> Q\[Custom Output\]' "$file_path"; then
        echo "❌ LOST CONTENT: Custom flowchart TB content not preserved in $file"
        FAILED_CHECKS=$((FAILED_CHECKS + 1))
      fi
      ;;
    *)
...
# Check for correct color scheme in diagrams (Deep Purple scheme)
if ! grep -q "classDef userRequirement fill:#D1C4E9,stroke:#7E57C2" "$TEST_DIR/specifications/Requirements.md"; then
  echo "❌ FAILED: Missing or incorrect userRequirement color definition in Requirements.md diagram"
  exit 1
fi

if ! grep -q "classDef systemRequirement fill:#E1D8EE,stroke:#673AB7" "$TEST_DIR/specifications/Requirements.md"; then
  echo "❌ FAILED: Missing or incorrect systemRequirement color definition in Requirements.md diagram"
  exit 1
fi

if ! grep -q "classDef verification fill:#DCEDC8,stroke:#4CAF50" "$TEST_DIR/specifications/Requirements.md"; then
